...
const vec3 lookinit(0.0, 0.0, 0.0); //Where the eye is looking at; should update with each passiveMousedrag callback
...
void keyboard(unsigned char key, int x, int y) {
	...
	//Obtain the forward direction in which the eye is pointing as a vector based in the origin
	vec3 eyeForward = look + (origin - eye);
	//Obtain the direction perpendicular to the one in which the eye is pointing as a vector based in the origin
	vec3 eyeSideways = glm::cross(up, eyeForward);

	switch (key) {
	case 'w':
		//Translate both the eye and the looking point by the same amount, so as to preserve the lookAt parameters
		eye = eye + eyeForward * SPEED;
		look = look + eyeForward * SPEED;
		break;
	case 'a':
		eye = eye + eyeSideways * SPEED;
		look = look + eyeSideways * SPEED;
		break;
	case 's':
		eye = eye - eyeForward * SPEED;
		look = look - eyeForward * SPEED;
...
void specialKey(int key, int x, int y) {
	...
	case 100: //left
		...
		break;
	case 101: //up
		Transform::up(amount, eye, up, false);
		break;
	case 102: //right
		Transform::left(-amount, eye, up);
		break;
...
void passiveMousedrag(int x, int y) {
	...
	if (x != centerX || y != centerY) {
		...
		//Translate eye towards origin, obtain look vector
		look = look + (origin - eye);
		//Rotate look vector
		Transform::up(-deltaY * SENSIVITY, look, up, true);
		Transform::left(-deltaX * SENSIVITY, look, up);
		//Translate eye back to origin, obtain new look point
		look = look + (eye - origin);
	}
	glutPostRedisplay();
}
...
int main(int argc, char* argv[]) {
	...
	glutPassiveMotionFunc(passiveMousedrag);
	//glutMotionFunc(passiveMousedrag);
